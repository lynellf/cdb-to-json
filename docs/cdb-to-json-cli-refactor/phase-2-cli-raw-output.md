# Phase 2 — CLI contract, raw profile, and secure publication
...
card. Reject before input access:
...
- `pretty` with JSONL;
- invalid profile/format/split/conflict/diagnostics values;
...
`openat`, `linkat` or an equivalent no-replace primitive, and `renameat2` where
required), file and directory destinations must use the native adapter. Acquire the
...
spools, reservations, locks, and temporary siblings. Cleanup order is: abort row
iteration; close writer; close SQLite; verify source snapshot; delete spool/index,
temporary files, and reservations; preserve any pre-existing final. A signal before
the synchronous commit barrier returns exit `6` without publication. A signal during
a completed barrier cannot roll back or misreport a committed final. The CLI
installs/removes one SIGINT handler around `convert()` and passes its signal.
...
4. Map diagnostics to exit codes with the documented precedence
   `2 > 3 > 4 > 5 > 6 > 7 > 1`; preserve source phase and stable machine envelopes.
...
1. Replace the in-memory `RawCardRows[]` path for raw output with a sequential
   reader-to-profile-to-writer pipeline. For raw, accumulate only the supported
   table rows required by one database envelope; do not retain all databases or
   merged card records. If a bounded envelope cannot satisfy the configured limits,
...
rounded/coerced, or a failed writer changes a pre-existing final.
...
- new `tests/cli/destinationLifecycle.test.ts`;
  `freshSplitRoot.test.ts`; `secureDestinationRace.test.ts`; and
  `pathSafety.test.ts`.

Tasks:

1. Define the native ABI in one small adapter. Remove or stop using path-based lock
   and rename helpers. The adapter must expose capability probe, trusted-root FD,
   descriptor-relative component traversal, exclusive lock/reservation, temp file,
   no-replace publication, force replacement, and cleanup. Ensure any native error
   maps to a stable destination diagnostic.
2. Correct the build matrix and manifest: Node 22+ Linux x64/arm64 is eligible,
   but a successful build must still prove the required kernel/filesystem primitives.
   Unsupported builds remove stale binaries and publish an explicit unsupported
   manifest. `package:check` must reject a manifest that claims support without a
...
4. Add reservations for locks, temp files, merge-private state, snapshots, and
   materialized files to one aggregate staging budget. Reserve before each write,
   reconcile actual growth, and clean reservations on every exit path.
5. Test state transitions and cancellation before, during, and after the commit
   barrier. Keep a prior final byte-identical on pre-commit abort or writer failure.
...
4. JSON and JSONL writers are incremental and deterministic; output limits and
   aggregate staging limits are enforced before writes and clean up private state.
5. File and split-directory outputs are atomic per logical unit, preserve prior
   finals on pre-commit failure, reject existing split roots, and implement the
   documented continue/merge semantics.
...
7. Snapshot/materialization, output staging, locks, and merge-private state share
   finite reservation accounting; `maxSnapshotBytes` and limit relations are
   enforced before input access.
...
- **Streaming/atomicity tension:** stdout is intentionally non-atomic; file and
  directory outputs are not. Do not weaken one contract to simplify the other.
