...
This is revision 12 of the tactical package for `docs/cdb-to-json-cli-refactor-spec.md`. It incorporates the plan-reviewer-a request for a concrete compatibility bridge, executable gates and fixtures, an explicit Phase 4 translator contract, and streaming/provenance verification; revision 2 addressed plan-reviewer-b's B1–B11 findings. Revisions 3–5 resolved adversarial findings B2-1 through B2-8 and R4-B1 through R4-B8 concerning output units, preflight resource limits, concurrent commits, cancellation barriers, source spans/snapshots, legacy replacement authorization, normalization conflicts, physical snapshots, UTF-8 fidelity, secure publication, and the async iterator. Revision 6 closes the follow-up review gaps by freezing `maxSnapshotBytes` in the CLI, making native build/package reproduction exact, declaring every Phase-0 gate script before use, freezing `UNSUPPORTED_DATABASE_ENCODING` in the initial diagnostic contract, and fixing aggregate schema filenames/identifiers. Revision 7 adds the implementation-checkpoint remediation package in `phase-0-1-remediation.md`; revision 8 closes the checkpoint review by making R4.2 UNKNOWN/null orphan states and post-probe native cleanup executable. Revision 9 closes reviewer-B's contract, native-publication, snapshot-lifecycle, and phase-gate findings: R6.2 now re-proves orphan fields through `$ref`-linked aggregates, R6.1 freezes an import-safe four-vector native build seam, the named-temp `openat`/`renameat2` ABI is selected, `sourceHandle` owns regular-member streaming snapshots and URI immutable/default-safe-integer extraction, and P2 evidence excludes deferred merge/publication work. Revision 10 replaces the former numeric exit-code chain with an explicit predicate matrix, makes raw envelope serialization table-streaming with a raw memory gate, defines the physical-reorder canonical-data projection, and bounds extra-table metadata counts. Revision 11 closes B-REVERIFY-1 through B-REVERIFY-4 at the wording level. Revision 12 makes source anchors and same-FD verification executable, selects a pre-write SQLite materialization quota bound, adds durable identity-guarded commit recovery, hardens legacy publication, and blocks phase dispatch on fresh named-suite evidence. The root spec remains normative for product behavior; these documents make its delivery executable.
...
| B-REVERIFY-1 source TOCTOU | held no-symlink parent/member descriptors, fstat identity verification, copy/hash from descriptor, and deterministic main/WAL/SHM swap fixtures |
| B-REVERIFY-2 serialized spans/physical identity | frozen `text.sections`/`text.sourceSpans` slices with copied text/start/end/kind/basis, `$ref` aggregate validation, and removal of `identity.databaseSha256` |
| B-REVERIFY-3 snapshot budget | dedicated chunk-level `maxSnapshotBytes` counter for copied/materialized/generated-private files plus growth/overflow fixtures |
| B-REVERIFY-4 multi-output rollback | descriptor-relative commit-set journal, reverse rollback/force backups, explicit recovery state, and injected between-final failure gates |
...
- Without `--continue-on-error`, all logical outputs are staged privately and published by one commit-set transaction: all reservations are held, a descriptor-relative journal records each final and any force backup, and a failure between final publications rolls back already-published entries in reverse order before returning exit 6. A successful set removes the journal/backups only after every final is committed. With it, unmerged `split=database` commits successful database files independently and removes failed files; merged runs never commit their single final stream until every input succeeds, so a mixed merged run leaves no new final. A writer failure or cancellation before the commit-set barrier always removes active staging and returns exit 6. A post-barrier signal cannot roll back a committed set.
...
- `text.raw` is exact source text; `text.normalized` and `text.normalizationVersion: "text-normalization/1"` are required, and the serialized `text.sections`/`text.sourceSpans` layout below carries inclusive/exclusive UTF-16 code-unit spans with `spansBasis: "normalized"` and `offsetEncoding: "utf-16-code-units"`; begin/end-only or root `text.spans` shapes are not valid alternatives;
...
`ygo.card-source/1` requires both `text.raw` and `text.normalized`. `text.raw` is the exact SQLite description, including original line endings and Unicode code-unit sequence. `text.normalized` uses `text-normalization/1`: CRLF and CR become LF, and Unicode NFC is applied; no other rewriting is permitted. The serialized source-span shape is frozen as `text.sections` plus `text.sourceSpans`: `sections` requires `material`, `pendulumEffect`, `monsterEffect`, `spellTrapEffect`, and `flavor` (`TextSlice | null`), `unclassified` (`TextSlice[]`), and `segmentation` (`EXACT_MARKERS | FRAME_RULE | PARTIAL | UNSPLIT`); `sourceSpans` is the complete ordered flattened list. Each `TextSlice` requires `{text, start, end, kind, basis}`, where `start` is inclusive and `end` exclusive, `start/end` are JavaScript UTF-16 code-unit offsets into `text.normalized`, `text` equals `text.normalized.slice(start, end)`, `kind` names the section meaning, and `basis` is exactly `"normalized"`. The document includes `text.normalizationVersion: "text-normalization/1"`, `text.spansBasis: "normalized"`, and `text.offsetEncoding: "utf-16-code-units"`. `text.spans` and `begin` are rejected serialized shapes; a span cannot omit its copied slice text. When no description exists, all nullable sections are null, `unclassified` and `sourceSpans` are empty, and segmentation is `UNSPLIT`. A generic consumer never applies normalized offsets to `text.raw`. Raw-only profile output remains exact and has no derived span contract.
...
- raw profile envelopes stream `tables.datas` and `tables.texts` arrays from fixed table iterators (or a bounded two-pass table iterator) and never construct `RawCardRows[]` or an equivalent complete per-database collector;
...
- a generated high-cardinality fixture is converted in a child process with `--max-old-space-size=256`; the streaming gate compares output to a collected reference and asserts bounded operation without using an all-record `Array` in the unmerged writer path; the raw profile has a separate high-cardinality child-process gate proving its envelope writer does not collect all table rows;
...
`maxSnapshotBytes` is a dedicated aggregate snapshot/materialization budget covering the private copies of every captured main/WAL/SHM member, the materialized main, and every generated private SQLite journal/WAL/SHM/temporary sibling for the lifetime of source acquisition/materialization; it is not merely a pre-copy source-bundle cap. The pre-copy member-size sum is an early rejection, but every copy/write chunk reserves against the remaining snapshot budget before writing and reconciles actual growth afterward. `maxStagingBytes` is the aggregate private-conversion budget and covers those same physical snapshot files once, plus destination staging, locks, commit journals/backups, and merge state; a physical file is charged once to each applicable counter, never duplicated within a counter. Snapshot/materialization overflow emits `RESOURCE_LIMIT_EXCEEDED` before the write that would exceed it. `maxSpoolBytes` is an aggregate private-merge-storage budget, not merely a spool-file budget. The reservation counter covers the length-delimited record spool, sorted/index files, lineage metadata, and their temporary siblings. Every private write reserves encoded bytes before writing and reconciles against actual file sizes; index growth alone can therefore raise `RESOURCE_LIMIT_EXCEEDED`. Stdout is non-atomic and is charged against `maxOutputBytes` only. No destination is opened before text/row preflight succeeds.

`maxRowsPerTable` applies to every required and extra table. Required `datas`/`texts` counts use the fixed count preflight. For each extra table, metadata collection performs a safely quoted, value-free bounded probe equivalent to `SELECT 1 ... LIMIT maxRowsPerTable + 1` (the sentinel is computed as `BigInt(maxRowsPerTable) + 1n` without numeric coercion): it records an exact count when the table is within the limit, and emits `RESOURCE_LIMIT_EXCEEDED` with `details.limitCode: "MAX_EXTRA_TABLE_ROWS_EXCEEDED"` as soon as the sentinel row is reached. It never selects extra-table cells or runs an unbounded count, and no output is opened after this diagnostic.
...
The error-precedence contract is an explicit predicate matrix, not a numeric total ordering. `computeExitCode` receives typed state including `optionError`, `hasUsableInput`, `inputAccessStarted`, input/strict/resource/integer failure flags, `mergeCollision`, `outputError`, `cancelled`, continuation counts, and `internalError`, and selects the first applicable row:

| Predicate (evaluated in order) | Exit |
|---|---:|
| option validation or invalid limit relation | `2` |
| `!hasUsableInput && !inputAccessStarted && !inputError && !strictFailure && !resourceOrIntegerFailure && !mergeCollision && !outputError && !cancelled && !internalError` | `3` |
| output error or cancellation (after the two preflight rows) | `6` |
| input/schema/strict/resource/integer failure, when no output error/cancellation occurred | `4` |
| merge collision, when no output error/cancellation or input failure occurred | `5` |
| continued processing with at least one completed and one failed input | `7` |
| unexpected internal failure | `1` |
| otherwise successful completion | `0` |

Thus output failure/cancellation overrides pending input, collision, or partial state; input failure overrides collision when no output operation failed; collision overrides partial success. A pre-open output failure is still exit `6`, not no-input, while a missing/empty discovery with no output failure is exit `3`. The implementation records all diagnostics even when the first terminal code is selected.
...
The application conversion state is `READING -> READY -> COMMITTING -> COMMITTED` or `ABORTED`. `better-sqlite3` iterators check `AbortSignal` every row and await `scheduler.yield()` at least once in each 256-row window (and before each commit), allowing SIGINT to run despite synchronous SQLite iteration. Before `COMMITTING`, a commit-set journal is created for every newly produced final and every force-replaced final; it names the reservation token, staged sibling, optional backup, and publication state. The commit protocol holds every reservation, publishes descriptor-relatively, and on an injected or real failure rolls back already-published entries in reverse order: no-force removes only this run's new finals, while force restores backups byte-for-byte. A rollback failure retains the journal/backups for explicit recovery and returns exit 6 rather than claiming success; ordinary injected failure tests must prove complete restoration and cleanup. A signal observed before the barrier aborts, closes writer then database, removes staging/spool/locks/journal/backups, preserves every existing final, and returns exit 6. Once the commit-set barrier begins it completes and marks `COMMITTED`; a signal delivered during or after it does not roll back a committed set or misreport it as cancelled. `--continue-on-error` remains the explicit per-database atomic exception for unmerged `split=database`; it does not promise a global commit set.
...
The database SHA-256 identifies a stable private source snapshot bundle, not an unqualified live main-file read. The bundle hash is the canonical SHA-256 of the exact ordered `[ {name,present,bytesBase64}, ... ]` tuple for `main`, `-wal`, and `-shm`, including explicit absent members; the equivalent canonical JSON hash is computed incrementally with bounded base64 carry state. Before opening a database, `sourceHandle` acquires the input directory through a held descriptor and traverses parent components with `openat2(RESOLVE_BENEATH|RESOLVE_NO_SYMLINKS)` or equivalent descriptor-relative `openat(O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC)` operations. Each source member is then opened and held with `O_NOFOLLOW|O_RDONLY|O_CLOEXEC` (or an equivalent no-symlink descriptor primitive), fstat-verified as the same regular member observed by lstat, and copied/hashed only from that descriptor. A path-based copy after lstat is forbidden. A deterministic lstat-to-open swap test covers `main`, `-wal`, and `-shm`: a symlink or changed inode yields `SOURCE_MUTATED_DURING_READ`/stable path diagnostic and never reads the replacement target. Before opening a database, `sourceHandle` captures/copies the main file plus every present `-wal`/`-shm` sidecar and rechecks original identities/bytes; a mutation, appearance, or disappearance retries and then emits `SOURCE_MUTATED_DURING_READ`. It opens only the copied bundle with ordinary WAL-capable access, escapes the private `VACUUM INTO` destination as a verified SQLite literal, inventories generated private SQLite sidecars, closes that connection, and opens only the materialized file through a `uri:true` URI with `immutable=1` and `defaultSafeIntegers()` for extraction. Extraction never replays WAL or creates a sidecar. A valid stable WAL bundle is supported; if safe descriptor traversal is unavailable or replay/materialization fails, the input fails before value selection (`INVALID_PATH` for unsafe source traversal or `CDB_OPEN_FAILED` for materialization), never falling back to a live/path read.
...
14. Repeated conversions prove deterministic record order, canonical JSON, source-revision/provenance hashes, and merge lineage; physically reordered fixtures compare equal under the named canonical-data projection and differ only in declared physical identity fields.
...
- `docs/cdb-to-json-cli-refactor/execution-contract.json` — canonical machine-readable execution agreement for phase dispatch and review;
...
The source hash covers a stable snapshot bundle, not an unqualified live main file. Snapshot acquisition runs before the database is opened: capture the main file and any present `-wal`/`-shm` member identities, reject symlinks/non-regular members, stream-copy all present members into a private sibling bundle, incrementally hash the exact ordered bytesBase64 tuple, reserve bytes, and re-stat/re-hash the original members. A member appearing, disappearing, changing identity/size, or changing bytes during acquisition retries a bounded number of times and then emits `SOURCE_MUTATED_DURING_READ`; it never silently drops a sidecar. The copied bundle is opened only with ordinary WAL-capable access, a verified SQLite literal is used for `VACUUM INTO`, generated private journal/WAL/SHM files are inventoried, and the connection closes before URI+immutable/default-safe-integer extraction. After that connection closes, extraction opens only the materialized main file read-only with `immutable=1`; no extraction read replays WAL or creates a sidecar. The reader verifies copied member identities/bytes before publication and deletes snapshot/materialized/generated-private artifacts after provenance is retained. A stable WAL bundle, a main-only database, a present-but-empty/required-sidecar case, hostile staging names, acquisition-time sidecar mutation, materialization failure, and post-copy snapshot mutation are all fixtures. If replay/materialization cannot complete, the input fails with `CDB_OPEN_FAILED` rather than falling back to a live read.
...
Secure file and split-directory destinations are supported only on Node 22+ Linux with the native adapter, `openat2`, named descriptor-relative `openat` temporary/lock creation, and separate `renameat2` no-replace/force operations. This is the selected equivalent under the senior D5 decision; `O_TMPFILE`, `linkat`, and path-based lock/rename helpers are not used. Stdout remains available without that adapter. Unsupported platforms, kernels, filesystems, or missing native builds return `UNSAFE_DESTINATION_FILESYSTEM` (exit 6) before input discovery/opening; there is no path-based fallback.

### R5.2 Module and lifecycle contract

The implementation uses these concrete boundaries:

- `src/cdb/sourceHandle.ts`: owns the captured member descriptors and identities, private paths, physical hash, snapshot-budget reservations, materialization/extraction ordering, and idempotent cleanup; `snapshotBundle.ts` and `materializeSnapshot.ts` are helpers behind this lifecycle boundary. Source acquisition is descriptor-safe: the input parent is held/traversed without symlink resolution, each main/WAL/SHM member is opened with no-follow/read-only/close-on-exec flags, fstat-verified against the lstat observation, and copied/hashed from that held descriptor. No path-based copy is permitted after lstat.
- `src/cdb/snapshotBundle.ts`: captures and copies `main`, present `-wal`, and present `-shm` before any SQLite open; rejects symlink/non-regular members; retries bounded source mutation races; incrementally computes the exact bytesBase64 physical bundle hash; and owns source re-stat/re-hash verification.
- `src/cdb/materializeSnapshot.ts`: opens only the private copied bundle with ordinary WAL-capable SQLite access, escapes private `VACUUM INTO` paths as verified SQLite literals, inventories generated private journal/WAL/SHM siblings, closes the bundle connection, and reports `CDB_OPEN_FAILED` on replay/materialization failure. It never opens an original path and never falls back to a live read.
- `src/cdb/openDatabase.ts`: opens only a URI for the materialized main file with `uri:true`, `readonly:true`, `fileMustExist:true`, and `immutable=1`, calls `defaultSafeIntegers()` before value queries, disables extensions, and checks UTF-8 before text selection; extraction never opens a WAL/SHM member.
...
- `src/application/stagingBudget.ts`: owns two explicit counters: `maxSnapshotBytes` for the aggregate private snapshot/materialization lifecycle (copied main/WAL/SHM members, materialized main, and generated private SQLite journal/WAL/SHM/temporary siblings), and `maxStagingBytes` for all private conversion state. Every physical file is charged once to each applicable counter; each copy/write chunk reserves before writing and reconciles actual growth. Source-member size sum is an early pre-copy check, not the complete snapshot enforcement. Legacy staging and lock/commit-journal/backup state are covered by `maxStagingBytes` in P2/P3; merge spool/index/lineage is deferred to P3/P4.
...
The bundle hash and `sourceRevisionId` are physical-snapshot identities: canonical hash input is the ordered tuple of `main`, `-wal`, and `-shm` member names, explicit presence markers, and copied bytes. A source with equivalent logical rows but different SQLite physical insertion/layout therefore has different bundle hash and `sourceRevisionId`. Physical-reorder tests compare a named `canonicalDataProjection(record)`, which removes only physical identity fields (`cdb.card/2` `source.databaseSha256`; `ygo.card-source/1` `sourceRevisionId` and the sole physical hash `simulatorSource.database.sha256`). `identity.databaseSha256` is forbidden in the source schema and goldens; adding another physical hash path requires a contract revision. The projection includes IDs, exact supported raw facts, canonical data/text ordinals, normalized fields, diagnostics, semantic provenance, merge winner choice, and lineage ordering, and must remain identical; full serialized records intentionally differ only in the listed physical identity fields. `conversionOptionsHash` remains identical. No logical-source-revision hash is exposed.
...
`LimitsV1` contains `maxRowsPerTable`, `maxTextBytes`, `maxOutputBytes`, `maxStagingBytes`, `maxSpoolBytes`, and `maxSnapshotBytes`; defaults are respectively 1,000,000, 4 MiB, 2 GiB, 4 GiB, 2 GiB, and 4 GiB. `maxSnapshotBytes` is the aggregate private snapshot/materialization budget, not only the pre-copy source-bundle sum: copied source members, materialized main, generated private SQLite files, and their temporary siblings reserve each chunk before writing and reconcile actual growth. `maxStagingBytes` covers the same physical files once plus all other private conversion artifacts, including commit journals/backups. Every selected limit is included unchanged in the shared semantic-options object and both relevant reports/hashes. Relations are reject-only: equality and less-than are valid; greater-than is `INVALID_LIMIT_RELATION` exit 2; zero is valid and enforced.

Add stable diagnostics `INVALID_LIMIT_RELATION`, `CDB_OPEN_FAILED`, `INVALID_TEXT_ENCODING`, `OUTPUT_DIRECTORY_EXISTS`, and `UNSAFE_DESTINATION_FILESYSTEM` alongside the existing matrix. Exit selection uses the explicit predicate matrix above: option/limit relation first (`2`); no usable input only when no input access started and no output/cancellation/internal failure (`3`); output failure/cancellation (`6`); input/schema/encoding/text/integer/strict/resource failure when no output failure (`4`); merge collision when no output or input failure (`5`); mixed continued database failures (`7`); then unexpected internal failure (`1`). Output directory existence and secure-destination capability are checked before opening inputs, but are output failures (`6`), not silently reclassified as input failures.
...
- `tests/reader/walMaterialization.test.ts`: WAL-only committed row, source member immutability, no extraction sidecar, materialization failure/no publish, and descriptor-safe member acquisition ordering.
...
- `tests/reader/physicalSnapshotProvenance.test.ts`: physical bundle hash/source revision changes after physical reorder while `canonicalDataProjection(record)`, canonical rows/ordinals/winners, and `conversionOptionsHash` remain equal; full-record differences are limited to declared physical identity fields.
...
- `tests/reader/stagingSnapshotBudget.test.ts`: aggregate `maxSnapshotBytes` snapshot/materialization reservations (including growth after stat and generated-private overflow), aggregate staging reservation/reconciliation, cleanup, and extra-table/WAL over-budget rejection; the test must prove chunk reservations rather than only the pre-copy sum.
...
- `tests/reader/sourceMemberSwap.test.ts`: injected lstat-to-open swaps for main, `-wal`, and `-shm`; no-follow descriptor open rejects the replacement symlink/identity and no target bytes are read.
- `tests/cli/commitSetRollback.test.ts`: injected failure between output publications, reverse rollback for no-force and force (including byte-identical restoration of existing finals), journal/backups/reservation cleanup, and rerun after rollback.
- `tests/cli/commitSetRecovery.test.ts`: rollback failure retains a recovery journal/backups and returns exit 6 without claiming an atomic success.
...
## Revision-8 checkpoint remediation: orphan nullability and post-probe cleanup

This is a narrow correction to the Phase 0–1 checkpoint. It does not reopen the accepted raw boundary, profile matrix, native support matrix, or senior decisions. R4.2 is authoritative for incomplete records: the prior absent-not-null fixture interpretation is rejected.

### Orphan contract

A datas-only and texts-only row keeps its present ID and raw side, sets the missing partner to `null`, and carries its stable missing-partner diagnostic. Card-profile golden records MUST emit the R4.2 nullable state rather than omit fields: `name` is `null` when texts are missing; `cardKind` is exactly `"UNKNOWN"`; `traits` is `[]`; `typeLine`, `monster`, `spell`, and `trap` are `null`; and the required card text object uses nullable text surfaces (with `text.raw: null` when no texts row exists). A texts-only card retains the exact name/text values and still uses `UNKNOWN` plus null/empty data-derived surfaces.

Source-profile golden records MUST emit `printed.name: null` for datas-only rows, `printed.cardKind: "UNKNOWN"`, null typed surfaces, a required text object with nullable `raw`/`normalized` values, and `simulatorSource.rawRows` with exactly one partner set to `null`. A texts-only source retains exact text values while data-derived fields remain null or empty. `references` remain empty and `coverage.status` remains `SOURCE_ONLY`; no placeholder semantic field is allowed. The item schemas must require these nullable/empty states so schema validation cannot accept an omitted-field alternative.

The P1 fixture/conformance scope is schema and golden-record closure. Mapper behavior remains owned by the later card/source phases, which consume these same fixed states; those phases must not revert the fields to omission.

### Native post-probe behavior

`build-native` has two unsupported branches: an unsupported host before build and a supported host whose compiled module fails the capability probe. Both branches MUST remove `dist/native/secure_destination.node` before writing `dist/native/capability.json` with `supported: false`; the second branch is not allowed to leave a module beside an unsupported manifest. A successful supported probe alone may retain the module and its matching SHA-256. `package:check` remains a defense-in-depth rejection for any unsupported manifest/module pair.

### Checkpoint acceptance

The checkpoint is accepted only after the updated orphan schemas/fixtures/tests pass and the exact native/build/package sequence passes:

```bash
npm run build:native && npm run build && npm run package:check
npm run test:fixtures && npm run test:conformance
npm run test:unit && npx vitest run tests/cli/limitRelations.test.ts
npm test
```

The focused commands prove the changed contract; `npm test` is the full repository gate and must be reported separately if an unrelated untracked test prevents a full-suite verdict. Stop rather than weakening R4.2, retaining a stale native module, or enabling path-based fallback. The previous P1 evidence is superseded until these commands are rerun.

## Revision-11 targeted adversarial closure (B-REVERIFY-1 through B-REVERIFY-4)

This addendum is normative and supersedes any earlier less-specific wording in this package. It is a targeted revision only; it does not reopen product scope, schema identifiers, the Linux publication decision, the raw boundary, or the accepted senior D1–D8 decisions.

### R11.1 — Descriptor-safe source acquisition

`sourceHandle` MUST acquire the input directory through a held directory descriptor. Every parent component is traversed without following symlinks using `openat2` with `RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS`, or an equivalent descriptor-relative sequence of `openat(O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC)` calls. Each `main`, `-wal`, and `-shm` member is opened with `O_RDONLY | O_NOFOLLOW | O_CLOEXEC` (or an equivalent no-symlink primitive), remains held for the complete copy/hash, and is fstat-verified as the same regular-file identity, size, and mode observed by the corresponding `lstat`. Copying or hashing through a path after `lstat`, reopening by an unchecked string, and following a member symlink are forbidden.

A member that is absent remains the explicit absent tuple entry. A symlink, non-regular member, parent-component swap, identity/size mismatch, appearance, disappearance, or byte mutation emits the stable path/mutation diagnostic (`INVALID_PATH` for unsafe traversal or `SOURCE_MUTATED_DURING_READ` for an acquisition race), closes all descriptors, removes private artifacts, and never reads a replacement target. `tests/reader/sourceMemberSwap.test.ts` injects a deterministic lstat-to-open swap for each of `main`, `-wal`, and `-shm`; it asserts no bytes from the replacement target are consumed and that cleanup completes. If the safe descriptor primitive is unavailable, source acquisition fails before SQLite open rather than falling back to a path copy.

### R11.2 — Frozen serialized source spans and physical identity

The source item and aggregate schemas, source goldens, and generic JSON/JSONL consumer all use one serialized shape: `text.sections` plus `text.sourceSpans`. `sections` has required `material`, `pendulumEffect`, `monsterEffect`, `spellTrapEffect`, `flavor`, `unclassified`, and `segmentation` members; the first five are nullable `TextSlice` values, `unclassified` is an array, and `segmentation` is one of `EXACT_MARKERS`, `FRAME_RULE`, `PARTIAL`, or `UNSPLIT`. `sourceSpans` is the complete ordered flattened list. Each `TextSlice` requires `text`, inclusive `start`, exclusive `end`, `kind`, and `basis: "normalized"`; its copied `text` MUST equal `text.normalized.slice(start, end)` using UTF-16 code-unit offsets. The root `text.spans` and `begin`/`end`-only variants are rejected with `additionalProperties: false`/required-field checks. No derived span is emitted when normalized text is null; orphan text uses empty spans and `UNSPLIT`.

The source schemas and goldens MUST remove `identity.databaseSha256`. The only source-profile physical hash path is `simulatorSource.database.sha256`; `canonicalDataProjection(source)` removes that path and `sourceRevisionId`, while retaining all logical identity, raw facts, ordinals, diagnostics, semantic provenance, and lineage. Aggregate validation MUST reach the frozen item schema through `$ref`, and negative mutations for missing `text`, `start`, `end`, `kind`, or `basis`, legacy `begin`, and the extra physical identity path MUST fail for both item and aggregate documents.

### R11.3 — Aggregate snapshot/materialization accounting

`maxSnapshotBytes` is the aggregate budget for the private snapshot/materialization lifecycle: copied main/WAL/SHM member bytes, the materialized main, generated private journal/WAL/SHM files, and any snapshot/materialization temporary sibling. It is not only the pre-copy source-bundle sum. The pre-copy sum is an early rejection; the implementation then reserves every copy/write chunk against a dedicated snapshot counter before writing and reconciles actual size after each write, so a source that grows after `stat` and a materialized/generated-private file that crosses the budget fail with `RESOURCE_LIMIT_EXCEEDED` before the overflowing write. The aggregate `maxStagingBytes` counter covers each of those physical files once plus destination staging, locks, commit journals/backups, and merge state; no physical file is double-counted within one counter. `maxSnapshotBytes > maxStagingBytes` remains a reject-only option error.

The named staging gate adds growth-after-stat, materialized-main overflow, generated-private overflow, and cleanup vectors. It must inspect reservation events or an injected budget rather than only checking a preflight sum. The semantic options object and diagnostics report both selected limits unchanged.

### R11.4 — Multi-output commit-set transaction

For a no-continue conversion that creates more than one file/directory final, atomicity is a commit-set contract, not merely a per-file claim. Before publication, the application holds every final reservation and writes a descriptor-relative commit journal containing the run token, ordered final names, staged siblings, prior-final presence/digests, optional force-backup names, and per-final state. It then publishes in deterministic order. On a failure between publications, reverse rollback removes only newly published no-force finals and restores every force backup byte-for-byte; reservations remain held until rollback and cleanup complete. On successful completion, the journal and backups are deleted only after all finals are committed.

`--continue-on-error` for unmerged `split=database` remains the explicitly documented per-database exception, and merged output remains one final transaction. A cancellation or writer failure before the commit-set barrier is exit 6 and leaves every prior final unchanged. If rollback itself fails, the journal and backups are retained for explicit recovery, the diagnostic identifies the recovery artifact, rerun refuses to adopt or mix that journal, and exit 6 is returned; the implementation must never claim that an all-or-nothing commit succeeded. `tests/cli/commitSetRollback.test.ts` injects a failure after each publication position for no-force and force and proves reverse restoration/cleanup; `commitSetRecovery.test.ts` proves the explicit recovery outcome. A signal after the barrier cannot undo a committed set.

### Revision-11 focused gate

Before implementation dispatch, run the existing package gates plus these targeted checks:
...
The package remains blocked if any source member is copied through a path after lstat, if a source span can be serialized as `begin`/`end` without copied text, if `identity.databaseSha256` remains in the source contract, if `maxSnapshotBytes` is enforced only before copying, or if a late multi-output publication can leave an undocumented partial set.

## Revision-12 enforcement addendum (supersedes conflicting R11 wording)

The complete corrective phase is `phase-r11-enforcement-and-gates.md` and the
materialization bound is specified in `materialization-quota.md`. Those files
are normative extensions of this spec. In particular:

- source acquisition has an explicit absolute/relative held-anchor algorithm;
  member FDs remain open through copy and post-copy re-hash, and parent swaps,
  same-inode mutations, sidecar changes, and unsafe platform traversal fail
  closed before SQLite;
- `maxSnapshotBytes` is admitted by a finite SQLite page/WAL-derived bound for
  the materialized main and every generated private file before writable
  materialization starts. Post-operation inventory and injected reservation
  events are supplementary only, not enforcement evidence;
- commit journals are fsynced before and between publications, record complete
  staged/published/backup identities, identity-guard reverse rollback, and
  explicitly refuse orphaned journals after process interruption with
  `OUTPUT_RECOVERY_REQUIRED` (exit 6); and
- legacy output uses the same held-root/no-follow descriptor-relative native
  publication matrix as modern file output. Recursive path operations and
  cross-device copy fallbacks are forbidden; output-bearing legacy calls fail
  closed on unsupported filesystems.

P1/P2 remain blocked until the named R12 files and tests exist, the authoritative
scripts fail on missing suites (without `--passWithNoTests`),
`docs/implementation/current.md` no longer claims unaccepted phases are
complete, and `scripts/phase-dispatch-check.mjs` has recorded a fresh passing
P1/P2 evidence record. The existing stale source schemas and historical gate
