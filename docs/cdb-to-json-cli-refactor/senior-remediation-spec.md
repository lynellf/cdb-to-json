...
Canonical signed-int64 ID ordering controls only record order, data/text ordinals, merge winner selection, normalized record content, and `conversionOptionsHash`. Tests that reorder physical inserts MUST expect identical canonical records/ordinals/merge winners and a changed bundle hash and `sourceRevisionId`. The converter exposes no logical-source-revision hash in v2.

### D2 — WAL is materialized before immutable extraction (R4-B2)

A stable source bundle is captured first. Snapshot acquisition copies `main`, and the exact present `-wal`/`-shm` members, while detecting source changes as already specified.
...
`LimitsV1` adds `maxSnapshotBytes`, default 4 GiB. The pre-copy sum of present source bundle member sizes must not exceed it or the aggregate `maxStagingBytes`. The snapshot bundle, materialized main, and their temporary members reserve and reconcile against aggregate staging before each copy/write. Snapshot private files are removed on every success/failure/cancellation after provenance is no longer required.
